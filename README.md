...
### データ規模 (Phase 3 時点)
14開催日・168レース・1,515エントリ・535ユニーク馬・**連続2走ペア 960件**
(範囲: 2026-04-17 〜 2026-05-11)

### 単独変数の影響度（タイム変化との Pearson 相関、n=960ペア）

| 変数 | 相関 r | 解釈 |
|---|---|---|
| **馬場水分量変化** | **-0.613** | **最強** ・湿るほどタイム短縮 |
| 着順変化 | +0.426 | 中程度 |
| 中休み日数 | -0.390 | 中程度・長期休みほどタイム短縮 |
| 積載重量変化 | +0.213 | 弱い・重量増でタイム延長 |
| **馬体重変化** | **-0.026** | **ほぼ無相関** (単独では効かない) |

### 水分量変化帯 × タイム変化 (200m基準)

| 変化幅 | n | 平均タイム変化 |
|---|---|---|
| 大きく湿る (+1.0以上) | 280 | **-22.1秒** |
| 少し湿る | 359 | -6.9秒 |
| 変化なし | 44 | -12.2秒 |
| 少し乾く | 211 | +6.1秒 |
| 大きく乾く (-1.0以上) | 66 | **+18.9秒** |

→ 水分量±1.0の変化で **±20秒** 動く。**モデル最重要特徴量**として確定。
...
| **±5kg以内** | 348 | **35.3%** (最良) |
| 増5-15kg | 210 | 30.5% |
| **増15kg以上** | 61 | **37.7%** (休養明け好調?) |

→ **U字型の関係**。単純な差分ではなく「±5kg内フラグ」「+15kg以上特例」が必要。

### 騎手交代の効果 (n=302交代 / 658継続)

- 騎手交代: 平均タイム短縮 -7.82秒、着順改善 -0.16
- 騎手継続: 平均タイム短縮 -6.55秒、着順悪化 +0.52

→ **1.3秒のタイム短縮効果**。ただしセレクションバイアス(成績不振→乗替)の可能性大。

### 過去3走平均着順 → 現走着順 (n=106)

- Pearson r = +0.239 (弱〜中程度の正相関)
- 過去成績はモデル入力として価値あり、ただし絶対ではない
